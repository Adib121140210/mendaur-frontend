...
export const activityLogService = {
  ...
  getUserLogs: async (userId, perPage = 20, page = 1) => {
    try {
      ...
      const data = await response.json()
      console.log(`📋 Activity logs for user ${userId}:`, data.data?.length || 0)
      return {
        success: true,
        data: data.data || [],
        pagination: data.meta || {}
  ...
  getByAction: async (action, perPage = 20, page = 1) => {
    try {
      ...
      const data = await response.json()
      console.log(`📋 Activity logs with action '${action}':`, data.data?.length || 0)
      return {
        success: true,
        data: data.data || [],
        pagination: data.meta || {}
  ...
  getByDateRange: async (dateFrom, dateTo, perPage = 20, page = 1) => {
    try {
      ...
      const data = await response.json()
      console.log(`📋 Activity logs from ${dateFrom} to ${dateTo}:`, data.data?.length || 0)
      return {
        success: true,
        data: data.data || [],
        pagination: data.meta || {}
